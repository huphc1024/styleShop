...
public class PublicIndexController {
	...
	public String cat(@PathVariable("slug") String slug, @PathVariable("id") String id, @RequestParam(value="page", defaultValue="1") int page, RedirectAttributes ra,
			ModelMap modelMap) {
		try {
	...
	public String thanhtoanok(@PathVariable("cid") int cid,RedirectAttributes ra,HttpServletRequest request, ModelMap modelMap, HttpSession session) {
	...
	public String thanhtoan(@Valid @ModelAttribute("objItem") CustomerDefault objItem, BindingResult rs, RedirectAttributes ra,HttpServletRequest request, ModelMap modelMap, HttpSession session) {
		...
		if(rs.hasErrors()){
			...
			System.out.println("alo3");
			Customer objC = new Customer(0, "", "", objItem.getHo(), objItem.getTen(), objItem.getAndress(), objItem.getEmail(), objItem.getTelephone(), "", 0, objItem.getCity(), objItem.getPostcode(), objItem.getCountry());
			try {
	...
	public String dangky(@Valid @ModelAttribute("objItem") Customer objItem, BindingResult rs,@RequestParam("re_password") String repass,ServletRequest request , RedirectAttributes ra, ModelMap modelMap) {
	...
	public String check(@RequestParam("account") String account, HttpServletRequest request,
	...
	public String login2(@RequestParam("username") String username, @RequestParam("password") String password, HttpServletRequest request, ModelMap modelMap,
	...
	public String logincheck(@RequestParam("username") String username, @RequestParam("password") String password, HttpServletRequest request, ModelMap modelMap,
			RedirectAttributes ra) {
		if("".equals(username) || username.length() < 6 ||"".equals(password) || password.length() < 6){
			ra.addFlashAttribute("title", "Đăng nhập");
			ra.addFlashAttribute("hrefhome", "/dang-nhap");
	...
	public String checkorders(@RequestParam("num") String num, @RequestParam("id") int id,ModelMap modelMap, HttpSession session){
		try {
	...
	public void danhgia(@ModelAttribute("objItem") Comment objItem, @RequestParam("noi_dung") String noi_dung,
		...
		for (Comment objComment : commentDao.getItemsByDetail(pid)) {
			...
			comment += "<div class='row'>" 
					...
					+ "src='/styleShop/templates/public/image/avatar_2x.png'>" 
					+ "</div>" 
					+ "</div>"
					+ "<div class='col-sm-11'>" 
					+ "<div class='panel panel-default'>" + "<div class='panel-heading'>"
					+ "<strong>Tên: " 
					+ objComment.getHo_ten() 
					+ "</strong> - <span class='text-muted'>" 
					+ date
					+ "</span>" 
					+ "</div>" 
					+ "<div class='panel-body'>" 
					+ objComment.getNoi_dung()
					+ "</div>" 
					+ "</div>"
					+ "</div>" 
					+ "</div>";
		}
		String html = "<div id='ajax'>" + "<div id='review'>"
				+ comment 
				+ "</div>"
				+ "<h2>Viết đánh giá</h2>"
				...
				+ "<span class='text-danger'>Chú ý:</span> Không sử dụng các định dạng HTML!" 
				+ "</div>"
				+ "</div>"
				+ "</div>" 
				+ "</div>";
		...
		if(session.getAttribute("listCart") == null){
			...
			if(listCart.size() < 1){
				String html = "<div id='cart' class='cart pull-right'>"
						...
						+ "	<h4>Giỏ hàng</h4>" 
						+ "	<a><span id='cart-total'>0 sản phẩm - 0 đ</span></a>" 
						+ "</div>"
						+ "</div>"
			...
			for (Product product : listCart) {
				...
				listProduct += "<li>" 
						...
						+ "		href='"+request.getContextPath()+"/"+slugUtils.toSlug(product.getName())+"-"+product.getId_product()+".html'><img"
						+ "			width='47px' height='47px' src='"+request.getContextPath()+"/files/"+product.getPicture()+"'"
						+ "			alt='"+product.getName()+"'" 
						+ "			title='"+product.getName()+"'></a></td>"
						+ "	<td style='width: 100px;' class='text-left'><a"
						+ "		href='"+request.getContextPath()+"/"+slugUtils.toSlug(product.getName())+"-"+product.getId_product()+".html'>"
						...
						+ "			onclick='return cartremove"+product.getId_product()+"();' title='Gỡ bở'" 
			...
			String html = "<div id='cart' class='cart pull-right'>"
					...
					+ "	<span class='icon-cart'></span>"
					+ "	<h4>Giỏ hàng</h4>" 
					+ "	<a><span id='cart-total'>"+sp+" sản phẩm - "+df.format(tong)+"đ</span></a>" 
					+ "</div>" 
					+ "</div>"
					+ "<ul class='dropdown-menu pull-right'>" 
					+ listProduct
					...
					+ "		<td class='text-right'>"+df.format(tong)+" đ</td>" 
					+ "	</tr>" 
					+ "	</tbody>" 
					+ "</table>"
					+ "<p class='text-right'>"
					...
					+ "		href='"+request.getContextPath()+"/thanh-toan'" 
					+ "		class='button btn btn-theme-default'><i"
